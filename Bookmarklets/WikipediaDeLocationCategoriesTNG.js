...
for (var i = 0; i < states.length; i++) {
  ...
  if (indexOfCurrentState > -1 && indexOfCurrentState <= closestValue) {
    /* catching Sachsen-Anhalt after Sachsen*/
    statePosition = i;
...
var hasVerbandsgemeinde =
  intro.indexOf("Verbandsgemeinde") > -1 && theState.indexOf("Sachsen") == -1;
...
if (parentPlace != "") {
  ...
  for (var i = 0; (aTag = document.getElementsByTagName("a")[i]); i++) {
    if (
      aTag.innerText.toLowerCase().indexOf("kreis") > -1 &&
      aTag.innerText != "Landkreis" &&
      aTag.innerText != "Kreis"
    ) {
      theCounty = aTag.innerText
        .replace("Landkreises", "")
        .replace("Landkreis", "");
      if (theCounty.indexOf("Kreis") == 0) {
        theCounty = theCounty.replace("Kreises", "").replace("Kreis", "");
      }
      ...
      aTag.innerText.startsWith("Verbandsgemeinde") &&
      aTag.innerText != "Verbandsgemeinde"
    ) {
